...
# 6. Smart Context Indexer (Custom @before_model - Hybrid Line Indexing)
@before_model
def smart_context_indexer(state: AgentState, runtime: Runtime) -> dict[str, Any] | None:
    """대용량 도구 출력이 감지되면 백그라운드 모델로 목차 및 줄 번호 인덱스를 생성하여 치환하는 미들웨어"""
    messages = state.get("messages", [])
    modified = False
    new_messages = []
    ...
    for msg in messages:
        if msg.__class__.__name__ == "ToolMessage" and len(str(msg.content)) > threshold:
            ...
            print(f"🔍 [Harness Middleware] Large Tool Output detected ({len(original_content)} chars). Compacting via SmartContextIndexer...")
            
            # 백그라운드 초고속 모델 기동 (Gemini-Flash)
            try:
                ...
                chain = prompt | llm
                # 문서가 너무 크면 모델 컨텍스트에 맞게 일부 슬라이싱하여 요약 호출
                toc_index = chain.invoke({"text": original_content[:60000]}).content.strip()
                
                compacted_content = (
                    f"[SYSTEM WARNING: 본문이 너무 길어 미들웨어에 의해 시맨틱 인덱싱 처리되었습니다. "
                    f"아래 목차를 참고하여 구체적인 정보 조회가 필요하다면 'file_read(file_path, offset=start_line, limit=limit)' 도구를 기동해 해당 영역을 상세히 읽어 질문에 답하세요.]\n\n"
                    f"{toc_index}"
                )
                
                # ToolMessage의 content 교체
                # ToolMessage 객체 속성을 새로 생성하여 교체합니다.
                msg = ToolMessage(content=compacted_content, tool_call_id=msg.tool_call_id)
                modified = True
            except Exception as e:
                print(f"⚠️ [SmartContextIndexer] 요약 인덱싱 실패: {e}")
                # 에러 발생 시 단순 헤드/테일 자르기로 폴백
                compacted_content = (
