...
본 디렉토리는 `agent_lab` 프로젝트의 핵심 하네스, 프롬프트 엔지니어링, 메모리 시스템 및 에이전트 통합 동작을 체계적으로 검증하는 테스트 모듈들을 관리합니다.
...
| **`tests/test_mission02.py`** | Mission 02 | `main_agent.py` 자가 복구 미들웨어 3종 & 도구 14종 결합 | `python tests/test_mission02.py` |
...
- **리포트**: [TEST_RESULTS.md](file:///c:/Users/hyoun/Desktop/working_project/agent_lab/tests/prompts%26memory/TEST_RESULTS.md) (최신 검증 결과 리포트)
- **세부 모듈**:
