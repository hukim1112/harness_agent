...
cells = [
    ...
    },
    {
        ...
        "source": [
            ...
            "load_dotenv(\"../.env\")\n",
            "sys.path.append(\"..\")\n",
            "sys.path.append(\"../src\")  # ← harness 패키지가 위치한 src 폴더 경로 수혈\n",
            "print(\"✅ 가드레일 실습을 위한 환경 변수 및 임포트 경로 로드 완료.\")"
        ]
    },
    {
        "cell_type": "markdown",
    ...
    },
    {
        ...
        "source": [
            ...
            "    print(f\"입력: {txt}\")\n",
            "    print(f\"🛡️ Llama Guard 판정: {judgment}\\n\")"
        ]
    },
    {
        "cell_type": "markdown",
    ...
    },
    {
        ...
        "source": [
            ...
            "        return \"🛑 [NeMo Rail Blocked]: 저희는 당사의 서비스 범위에 최적화된 에이전트입니다. 타사 제품이나 외부 어시스턴트에 대한 성능 평가 및 비교 정보는 제공하지 않습니다.\"\n",
            "    \n",
            "    # 3. 정상 토픽인 경우 답변 생성\n",
    ...
    },
    {
        ...
        "source": [
            ...
            ")\n",
            "\n",
            "# 3. 적재된 로컬 감사 로그 데이터 중 \"에이전트 전체 실행 로그\"만 추출해 확인\n",
            ...
            "            # 이번 세션 ID 로그만 예쁘게 필터링해 확인 (JSONDecodeError 예외 방어 구현)\n",
            "            if log_entry.get(\"session_id\") == \"session_obs_002\" and log_entry.get(\"event\") == \"agent_execution\":\n",
...
with open("notebooks/05_guardrails_and_monitoring.ipynb", "w", encoding="utf-8") as f:
    json.dump(nb, f, indent=2, ensure_ascii=False)
print("[*] Successfully built notebooks/05_guardrails_and_monitoring.ipynb with 3-tier lightweight guardrail emulation.")
