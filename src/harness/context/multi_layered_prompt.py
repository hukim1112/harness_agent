...
- Course Material: 2.Agent Context Engineering.ipynb
- Architecture Notes: references/ref_02_context/architecture_notes.md

[5-Layer System Prompt Stack & Caching Architecture Principle]
-------------------------------------------------------------------------------
1. 정적 영역 (Layer 1 ~ Layer 2) -> KV Cache HIT 대상 (Static Prefix)
   - [L1: GLOBAL BASE]: 에이전트 정체성 및 런타임 기본 페르소나 (PROMPT.md / dynamic_l1)
   - [L2: TOOL SPECIFICATIONS]: request.tools 도구 JSON Schema 및 Skills 카탈로그
   - 이 영역들은 경계선(__SYSTEM_PROMPT_DYNAMIC_BOUNDARY__) 윗단에 고정되어 GPU KV Cache에 영구 적치됩니다.

2. 동적 환경 영역 (Layer 3 ~ Layer 5) -> boundary 아래 위치 (Uncached Dynamic Suffix)
   - [L3: DYNAMIC_WORKING_CONTEXT]: 운영체제(OS), 타임스탬프, 턴수, 권한 등 런타임 환경
   - [L4: RECALLED_MEMORY]: 장기 기억(L2/L3) 및 세션 문서
   - [L5: LOCAL PROJECT RULES]: AGENT.md에 기술된 레포지토리 로컬 행동 강령 (멀티 프로젝트 캐시 오염 방지)
   - 변동성이 있는 L3~L5가 경계선 아래로 격리됨으로써 상단 L1~L2의 정적 글로벌 캐시 오염을 완벽히 방지합니다.

3. 사용자 최종 입력 (User Task) -> HumanMessage로 자동 분리 연동
   - 사용자가 입력한 최종 태스크(질문)는 시스템 프롬프트가 아닌 메시지 리스트(messages)의 
     HumanMessage 객체로 자연스럽게 전달됩니다.
===============================================================================
"""
